...
\end{center}
\begin{enumerate}
\item Find the Expected value,  variance, and MGF of the random variable {\bf X}.
\item Find  $\HLTW{E\left( \frac{1}{2+|{X}|} \right) }$
\item Find  $\HLTW{E\left( \sqrt{X+1} \right) }$
\item Find the CDF of the distribution and plot it. 
\end{enumerate}
}\\


\qbx[6.4in]{ceil!40}{\item 
Derive the MGF of the Poisson$(\lambda)$ distribution where $\lambda>0$. Use it obtain Mean and Variance of the distribution. 
}\\


\qbx[6.4in]{ceil!40}{\item 
Derive the Mean and Variance of a Binomial$(n, \binP)$ distribution where $1>\binP>0$, and $n\in \mathbb{Z}_{+}$.  
}\\


\qbx[6.4in]{ceil!40}{\item 
Derive the Mean and Variance of a Geometric$(\binP)$ distribution where $1>\binP>0$.  
}\\
...
\item  A tollbooth operator has observed that cars arrive randomly at a rate of 360 cars per hour
\begin{enumerate}
...
\begin{enumerate}
\item Obtain the value of $E(X)$, $\text{Var}(X)$, and $ E(X^2) $.
\item What is $E(\frac{1}{2}X^2-2X-1)$
\item What is the MGF of X?
\end{enumerate} 
}\\
