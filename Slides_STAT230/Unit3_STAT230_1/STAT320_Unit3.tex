...
Two events $A$ and $B$ are disjoint (or mutually exclusive) if $A\cap B=\Phi$.  A collection of events  $\{A_i\}_{i=1}^{n}$ are pairwise disjoint (or mutually exclusive) if $A_i \cap A_j=\Phi$ for all $i \neq j$.
\end{definition}
\pause
\define{Partition}{ $A_1,A_2, \ldots,  A_n$ are called partition of the sample space $S$ if $\{A_i\}_{i=1}^{n}$ is pairwise disjoint and $\HLTY{\displaystyle \cup_{i=1}^{n} A_i=S.}$}
\pause
\vspace{.1in}
\qBrd[4.6in]{teal!40}{
Comment: Any set $A$ and  it's complement,  $\Not{A}$,  creates  a partition of $S$.}
\pause
\qBrd[4.6in]{amethyst!40}{In the above definition we may replace $n$ by $\infty$ and the definition naturally extends. }
...
\TransitionFrame[bittersweet]{\Large Permutations  }
