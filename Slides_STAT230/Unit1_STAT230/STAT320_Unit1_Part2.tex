...
\tiny 
\begin{itemize}
\item $\frac{d}{dx}x^2e^{x}=$
\item $\frac{d}{dx}e^{30x^2}=$
\item $\frac{d}{dx}xe^{x^2}=$
\end{itemize}
\vspace{3in}
\end{frame}
...
\begin{frame}\frametitle{Gamma Function, $\Gamma(\alpha), \alpha>0$}
 ...
 \begin{itemize}
 \item $\Gamma(\alpha)>0$ for all $\alpha>0$.
\item $\Gamma(\frac{1}{2})= \sqrt{\pi}$.
 \item $\Gamma(1)= 1$
 \end{itemize}
 \vspace{.1in}
\begin{itemize}
\item $\displaystyle \Gamma(\alpha)= (\alpha-1) \Gamma(\alpha-1)$ for any $\alpha>1$
\item $\Gamma(n)= (n-1)!$ when $n$ is a positive integer.
\end{itemize}
\vspace{1in}
\end{frame}
...
\begin{center}
\qBrd[4in]{cadmiumorange!50}{ $\HLTW{\HLTY{\mathscr{B}\left(\alpha, \beta\right)}:= \displaystyle \int\limits_{0}^{1}  x^{\alpha-1}(1-x)^{\beta-1} dx} $ for $\alpha>0, \beta>0$.}
 \vspace{.1in}
 
 \qBrd[3.8in]{tangerine!50}{
 $ \mathscr{B}\left(\alpha, \beta\right)$ is often calculated using the following equation: 
 $$\HLTW{\displaystyle \mathscr{B}\left(\alpha, \beta\right):= \frac{\Gamma(\alpha)\Gamma(\beta)}{\Gamma(\alpha+\beta)}}$$
...
\begin{center}
\qBrd[4in]{applegreen!40}{ $\HLTW{\HLTY{\Phi\left(x \right)}:= \displaystyle \int\limits_{-\infty}^{x}  \frac{1}{\sqrt{2\pi }}e^{-\frac{z^2}{2}} dz} $ for all $x\in \R$.}
\begin{itemize}
 \item $\Phi(0)=\frac{1}{2}$
