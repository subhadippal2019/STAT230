...
consideration.
  \end{defn}	 
	 \vspace{.05in}
 \qbx[4.5in]{teal!40}{
\qBrd[4.4in]{white!40}{ In the context of probability theory,   it is often denoted by $\HLTY{\SampleS}$ (or $\Omega$)}\\
\qBrd[4.4in]{lime!40}{ The universal set contains all of the elements relevant to a
given discussion.}}\\
\begin{center}
\includegraphics[scale=.27]{figs/SampleSpace.png} 
\end{center}
...
\vspace{-.2in}
 ...
 The logical operator is \HLTY{\text{NOT} }.
    ...
    \includegraphics[scale=.32]{figs/PALess1.png} 
     \includegraphics[scale=.23]{figs/VDblankOneSets2.png} 
  	\end{center}
	\vspace{4in}
	\end{frame}
...
\begin{frame}
	...
	\qBrd[4.6in]{apricot!40}{\qBrd[.4in]{apricot}{\bf Tuple:} A {\bf $\HLTY{k}$-tuple} is the a the ordered sequence  of  $k$ values that are written inside a parenthesis while different elements are separated by comma. 
	}\\
...
\end{frame}
 ...
 \qBrd[4.5in]{tangerine!40}{\sqBullet{orange}$\HLTW{\infty}$ is not a real number.  It is a concept to denote the supremum in the real number system. We may think it to be larger than any given/prespecified number.  Below are a few of its  convensions/properties.}
...
%\vspace{.5in}
\begin{itemize}
\item $\infty + x= \infty $ for any $x\in \R$
\item $-\infty + x= -\infty $ for any $x\in \R$
\item $\infty + \infty= \infty $ 
\item $-\infty - \infty= -\infty $ 
\item \qBrd[2in]{red!40}{$\HLTW{\infty - \infty}= \text{Undefined} $ }
\end{itemize}
...
\section{A Discussion on Cardinality of Sets }
...
% \TransitionFrame[antiquefuchsia]{\Large Notation For Some Standard  Function}
