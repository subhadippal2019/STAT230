...
\small 
\Exmpl{slateblue}{} Two cards are selected randomly with replacement from the deck of 52 cards.
\begin{enumerate}[a).]
...
\qBox{ Consider all the permutations of the numbers $\HLTW{\{1, 2, \ldots , N\}}$.  Assume all its permutations are equally-likely.  What is the probability of selecting a permuation so that none of its digits match with that of a provided sequence.  For Example if the provided sequence is  {$(1,2,\ldots, N-1, N)$ },  then the $i^{\text{th}}$ digit of the selected sequence can not be $i$, for $i=1, \ldots N$.
}
%\qBox{\Qn:
...
\item  Shat that  for $\HLTW{1\leq i_1\neq i_2  \leq N $,  $P(A_{i_1} \cap A_{i_2})= \frac{(N-2)!}{N!}}$. 
%\item  Show that  for $\HLTW{1\leq i_1\neq i_2 \neq i_3 \leq N  $,  $P(A_{i_1} \cap A_{i_2} \cap A_{i_3})= \frac{(N-3)!}{N!}}$. 

\item  In general,  argue that  for $\HLTW{1\leq i_1\neq i_2 \neq i_3 \neq i_k \leq N $,  $P(A_{i_1} \cap A_{i_2} \cap A_{i_3})= \frac{(N-k)!}{N!}}$, for $k=1, \ldots , N$
