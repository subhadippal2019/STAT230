...
\qBox{ Consider all the permutations of the numbers $\HLTW{\{1, 2, \ldots , N\}}$.  Assume all its permutations are equally-likely.  What is the probability of selecting a permuation so that none of its digits match with a provided sequence {$(1,2,\ldots, N-1, N)$ }
\begin{enumerate}[a).]
\item Define $A_i$ be the sequence that $i^{\text{th}}$ position matches with the provided sequence.   Prove that  $\HLTW{P(A_i)=\frac{(N-1)!}{N!}}.$ 
\item  Argue that  for $\HLTW{i_1\neq i_2 $,  $P(A_{i_1} \cap A_{i_2})= \frac{(N-2)!}{N!}}$. 
\item  Argue that  for $\HLTW{i_1\neq i_2 \neq i_3 $,  $P(A_{i_1} \cap A_{i_2} \cap A_{i_3})= \frac{(N-3)!}{N!}}$. 
...
queen of spades.  Let $\HLTW{A_i}$ denotes the event that there is a match at the $i^{\text{th}}$ turn.   
\begin{enumerate}[a).]
