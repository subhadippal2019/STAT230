...
\HLTW{\text{Sample Space:}} $\SampleS =$ The Set of all \HLTW{7!} permutations of (1, 2, 3, 4, 5,6,7)
}
\end{center}
{\tiny 
...
\pause
\includegraphics[scale=.3]{figs/SixesTable_7.png}
...
\Exmpl{babyblue}{} Suppose that A and B are mutually exclusive
...
\vspace{.1in}
\pause
\;\;\includegraphics[scale=.23]{figs/SixesTable_7.png}\\
%\vspace{1.5in}
{\tiny{ \bf Solution:} We shall solve this problem under the assumption that all of the 36 possible outcomes are equally likely.  Since there are 6 possible outcomes namely,  (1; 6); (2; 5); (3; 4), (4; 3); (5; 2), and (6; 1)that result in the sum of the dice being equal to 7, the desired probability is  $\frac{6}{36} =\frac{1}{6}$.}
\vspace{2in}
...
Consider 6 tosses of a fair coin.  For example two typical sequences of outcomes that are considered different is  `HTTTTT', and `THTTTT'.
\begin{enumerate}[a).]
\item What is the probability that there are be exactly 2 heads ?
\item What is the probability that there are no heads?
\item What is the probability that there are at least one Tail?
\end{enumerate} 
}\\
...
\begin{frame}
\vspace{-.15in}
\qBrd[4.55in]{purple!30}{ \Exmpl{purple!50}{} A fair-dice is thrown 5 times and all the the 5 numbers  that appear are recorded.  
\begin{enumerate}[a).]
\qBrd[4in]{applegreen!40}{ \item What is the probability that there is  no six?}
\qBrd[4in]{babyblue!40}{ \item What is the probability that there is at least one six?}
\qBrd[4in]{amber!40}{ \item What is the probability that there will  be exactly two four and 2 three?}
\end{enumerate}
\vspace{.05in}
} 
...
A typical ATM pin consists of 4 digits.  Assuming that all the integers between $0$ to $9$ are equally likely, for each of the digit selection. Find the probability of the following events. 
\begin{enumerate}
\item What is the probability that all the four digits of a randomly selected ATM pin is different?
\item *The bank has selected a group of 200 ATM users randomly.  What is the probability that atleast two of the users have exact same ATM pin
\end{enumerate}  }\\
