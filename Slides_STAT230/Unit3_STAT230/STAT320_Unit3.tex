...
\vspace{.1in}
Let $(\SampleS , P )$ be a sample space along with the Probability measure. Let $\HLTW{A}, \HLTW{B}$ be two events.  Then \\
\begin{enumerate}
\qBrd[3.5in]{applegreen!40}{  \item[{\sqBullet{olive}}] $P(\emptyset)=0$ where $\emptyset$ denotes the Null set. }\\
\vspace{.05in}
\qBrd[2in]{apricot!40}{ \item[{\sqBullet{apricot}}]  $P(A)\leq 1$. }\\
\vspace{.05in}
\qBrd[2.8in]{lime!40}{ \item[{\sqBullet{lime}}] If $A\subset B $ then $P(A)\leq P(B) $.}\\
\vspace{.05in}
\qBrd[3.8in]{brightlavender!40}{\item[{\sqBullet{amethyst}}] $P(\Not{A})=1-P(A)$, where  $\Not{A}$ denotes the complementary event to $A$}\\
\vspace{.05in}
\qBrd[4in]{cadmiumorange!40}{\item[{\sqBullet{cadmiumorange}}]  $P(A\cup B)= P(A)+P(B)-P(A\cap B)$.}
\end{enumerate}
}
\end{frame}
...
%\vspace{.05in}
\qBrd[4.5in]{amethyst!50}{\sqBullet{amethyst} If the sample space $\SampleS$ is finite and all the corresponding outcomes ( {\tiny elements in $\SampleS$})  are equally-likely,  then it follows from Axiom 3 that, for any event $E$,  
\vspace{-.15in}
\begin{center}
\qBrd[2.5in]{babyblue!50}{$\HLTW{\displaystyle P(E)=\frac{\text{Number of elements in E}}{\text{Number of elements in }  \SampleS}}.$}
\end{center}
\vspace{.05in}
}
...
\item[\sqBullet{amethyst}] To refer to the ``Equally Likely Setup'' we typically use the phrases such as the `Fair Coin' or `Fair Dice'
}\\

\vspace{.5in}
\qBrd[4.2in]{olive!50}{
\item[\sqBullet{olive}] For problems with Dice and Coin, if it is not Explicitly mentioned, we assume it to be a "Fair Coin" or "Fair Dice".  }
\end{itemize}
