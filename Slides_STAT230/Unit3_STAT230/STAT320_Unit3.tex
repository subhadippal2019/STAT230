...
\HLTW{\text{Sample Space:}}\\ $\SampleS ={\tiny\{H, T\}\times \{H, T\}}=\HLTY{ \{(H,H), (H,T), (T, H), (T,T)\}}$
}\\
\pause
\vspace{.01in}
...
Comment:} Any set $A$ and  it's complement,  $\Not{A}$,  creates  a partition of $\SampleS$.}

\pause
\vspace{-.15in}
\qBrd[4.7in]{amethyst!50}{\HLTW{
Comment:}In the above definition, we may replace $n$ by $\infty$ and the definition extends naturally. }
...
\define{Probability}{
Consider an experiment whose sample space is $\SampleS$. For each
event $E$ of the sample space $\SampleS$, we assume that a set-function
...
\TransitionFrame[bittersweet]{\Large A Few Examples: \\
{\tiny when $\SampleS$ is a finite set.  }
 }
...
\begin{frame}
\begin{itemize}
\item To refer to the ``Equally Likely Setup'' we typically use the phrases such as the `Fair Coin' or `Fair Dice'
\vspace{.5in}
\item For problems with Dice and Coin, if it is not Explicitly mentioned, we assume it to be a "Fair Coin" or "Fair Dice".  
\end{itemize}
