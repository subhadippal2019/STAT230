...
\begin{center}
\qBrd[3.5in]{applegreen!50}{$$ \HLTW{   P\left(\HLTEQ[amethyst!40]{A}\right)}:=  P(X\in  \HLTEQ[amethyst!40]{A})=  \HLTW{  \displaystyle \sum_{\left\{x\in \HLTEQ[amethyst!40]{A}\right\}} \HLTY{\pmf(x)}}.$$} 
\vspace{.1in}
\end{center}
}
...
\vspace{.1in}
\hspace{.5in}\includegraphics[scale=.3]{figs/SixesTable.png}\vspace{3in}
\end{frame}
...
\begin{frame}{Example: Check whether the examples are valid pmf}
...
\vspace{.1in}
\item What is the value of $\HLTY{1+\frac{1}{7}+\frac{1}{7^2} + \frac{1}{7^3}+ \cdots =}$\\
\end{enumerate}
\vspace{3in}
\end{frame}
...
$$ \HLTW{\displaystyle \HLTEQ[babyblue!40]{\HLTW{\pmf(x)}:=\displaystyle  \frac{\HLTY{c}}{5^x}} \text{ for } x= 0, 1, 2, 3, \ldots  } $$
where \HLTY{c} is an appropriately chosen constant. 
\begin{enumerate}
\item What is the value of \HLTY{ c} so that $\HLTW{\pmf(x)}$ become a valid probability mass function on the support $\support[X]$?
...
\begin{frame}{Reminder from Unit1}
...
\begin{frame}\frametitle{Exercises on Computing $E(X)$,  $Var(X)$, $MGF(X)$}

\qBrd[4.6in]{olive!30}{
\Qn: Let $X$ be a discrete random variable with the probability mass function $\pmf_{_X}(x)$ on the support $\support[X]$. Let $\HLTY{Y = \Indicator{A}{X}} $,  obtain $\HLTW{E(Y )}$,   $ \HLTW{\text{Var}\left( Y\right)}, $ and  $ \HLTW{\text{MGF}\left( Y\right)}.$
}
