...
\begin{frame}
\includegraphics[scale=.35]{figs/Binomial_three_all.png}
\end{frame}
...
\begin{frame}
\includegraphics[scale=.2]{figs/Pois_2.png}
\includegraphics[scale=.15]{figs/Pois_5.png}
\end{frame}
...
\begin{frame}
\includegraphics[scale=.2]{figs/Geo_.5.png}
\includegraphics[scale=.2]{figs/Geo_.2.png}
\end{frame}
...
\begin{frame}\frametitle{Reminder from Unit1: Geometric Series}
\qBrd[4.5in]{babyblue!50}{
Let $ \HLTEQ[amethyst!40]{p} \in \R$ be such that $\abs{\HLTEQ[amethyst!40]{p}}<1$,  then   $$  \HLTW{\HLTY{\displaystyle \sum_{y=1}^{\infty}\HLTEQ[white!40]y {\HLTEQ[amethyst!40]{p}}^{y}} =\HLTY{\displaystyle \frac{\HLTEQ[amethyst!40]{p}}{\left( 1-\HLTEQ[amethyst!40]{p}\right)^2  }}}$$ 
}\\
\vspace{.2in}
\qBrd[4.5in]{ceil!50}{
Let $ \HLTEQ[amethyst!40]{p} \in \R$ be such that $\abs{\HLTEQ[amethyst!40]{p}}<1$,  then   $$ \HLTW{ \HLTY{\displaystyle \sum_{y=1}^{\infty}\HLTEQ[white!40]y^2 {\HLTEQ[amethyst!40]{p}}^{y}} =\HLTY{\displaystyle \frac{\HLTEQ[amethyst!40]{p}}{\left( 1-\HLTEQ[amethyst!40]{p}\right)^2  }}+ \HLTY{\displaystyle \frac{\HLTEQ[amethyst!40]{2p^2}}{\left( 1-\HLTEQ[amethyst!40]{p}\right)^3 }}}$$ 
}
...
\qbx[4.5in]{olive!30}{
\Exmpl{olive}{}{
Approximately 10\% of the glass bottles coming off a production line have serious flaws in the glass.   20 bottles are randomly selected from the product liine. 
\begin{enumerate}
\item What is the probability that none of the selected bottles are flawed?
 \item What is the probability that exactly 10 of the selected bottles are flawed?
\item Find the expected number (mean) of flawed bottles  and the corresponding standard deviation.
\end{enumerate}
}  
}\\
\vspace{2.5in}
...
\qbx[4.5in]{airforceblue!30}{
\Exmpl{airforceblue}{}{
A company's toll-free complaints line receives an average of about 40 calls per hour.   Use the Poisson distribution to obtain the following:
\begin{enumerate}
\item what is the probability that there is no calls  during a 1-minute  the period?
\item what is the probability that at least 1 calls arrives during the 2 minute period?
\item what is the probability two or more calls arrives in a 2- minute perior?
\item What is the expected number of calls in a 20 minutes period.
\end{enumerate}
}  
}\\
\vspace{2.4in}
...
\qbx[4.5in]{apricot!50}{
\Exmpl{apricot}{}  
From a database of a tele-marketing company, It is know that only 20\% responds positively and agree to participate in their telephonic survey about a specific type of product.   A tele-marketing agent was assigned to a task where it is required for positive participation from  10 individuals.  
\begin{enumerate}
\item What is the probability that the agent needs to call 50 individuals to get a all these 10 positive responses?
\item  What is the expected number of individuals the agent needs to make calls to obtain 10 positive responses? 
\item  What would be the required expected number of individuals the agent needs to make calls to obtain 15 positive responses instead? 
\end{enumerate}	
}
\vspace{1in}
\end{frame}
...
\qbx[4.5in]{teal!30}{
\Exmpl{teal}{}{
Suppose that a lot of  electrical fuses contains 5\% defectives.  If a random sample of 10 fuses are tested, 
\begin{enumerate}
\item find the probability of observing at least one to be defective.
...
\qbx[4.5in]{teal!30}{
\Exmpl{amber!80}{}
An integrated circuit manufacturer produces wafers that
contain 18 chips. Each chip has a probability of 0.085 of
not being placed quite correctly on the wafer.
\begin{enumerate}
\item Find the probability that a wafer contains at least
three incorrectly placed chips.
\item  What is the probability that a wafer contains no more
than one incorrectly placed chip?
\item What is the expected number of incorrectly placed
chips on a wafer?
\end{enumerate}
}
\end{frame}
