...
\title{  STAT 320: Principles of Probability\\ {\color{black}   Unit 5: Discrete Random Variables}}
...
\TransitionFrame[bittersweet]{\Large Discrete Random Variables  }

\begin{frame}
\define{Discrete Random Variables}{A random variable that can take on at most a countable number of possible values is said to be discrete.}


\qBrd[4.5in]{babyblue!70}{
For a discrete random variable $X$, we define the probability
mass function (pmf) $\pmf_{_X}(a)$ of $X$ by
$$\pmf_{_X}(x)= P(X=x) \text{ for all } x\in \support[X]$$
}

The pmf of the random variable representing the sum when two dice are rolled can be represented in multiple ways.
...
The cumulative distribution function (cdf), denoted by $\cdf_{_X}(\cdot)$ is the following quantity
$$\cdf_{_X}(a)= P(X\leq a)= \sum_{\{x\leq a\}}\pmf_{_X}(a)$$	
}
\end{frame}
...
\Exmpl{amethyst}{}  If X has a pmf given by 
...
\vspace{-.1in}
\qbx[4.5in]{amethyst!50}{
Let the pmf of a random varibale $X$ is given as 
\begin{center}
\begin{tabular}{|l|l|l|l|}
\hline\hline
x    & 0    & 1    & 2    \\ \hline
...
\TransitionFrame[bittersweet]{\Large Expected Value and Variance  }
...
$$\DBX{ \displaystyle \mu_{_X}= E\left(\HLTW{X}\right)= \sum_{\left\{\HLTW{x}\in \support[X] \right\}} \HLTW{x}  \; \HLTY{\pmf_{_X}(x)}.}$$
}
\end{frame}
...
\begin{frame}
\define{\tiny The  Expected Value of a Function of a Discrete Random Variable}{Let the random variable $X$ has the probability mass funciton $\HLTY{\pmf_{_X}(x)}$ for all $x\in \support[X]$, the support of X.  Let $h(x)$ be any$^{*}$ function,  then the expected value of $ h(X)$ is defined as 
$$\DBX{ \displaystyle  E\left(\HLTW{h(X)} \right)= \sum_{\left\{\HLTW{x}\in \support[X] \right\}} \HLTW{h(x)}  \; \HLTY{\pmf_{_X}(x)}.}$$
}
\end{frame}
...
$$\DBX{ \displaystyle \text{Var}(X):= E \left(X-\HLTY{ \mu_{_X}} \right)^2},$$
\vspace{-.05in}
...
$$\DBX{ \displaystyle \text{Var}(X):= E(X^2)- \left(E(X) \right)^2}$$\\
\vspace{-.1in}
}

$\text{Var}(X)$ is often denoted by $\sigma^2$, i.e.  $\sigma^2=\text{Var}(X)$ and the standard deviation is $\sigma= \sqrt{\sigma^2}= \sqrt{\text{Var}(X)}.$\\
...
\item $\text{Var}(a+b X)= b^2 \text{Var}(X)$
\end{enumerate}
\end{frame}
...
$$\DBX{ \displaystyle \text{M}_{_X}(t):= E\left(e^{tX} \right)},$$
whenever it exists. 
}
\begin{center}
\qBrd[4.2in]{ube!40}{ If the random variable $X$ has the probability mass funciton $\HLTY{\pmf_{_X}(x)}$ for all $x\in \support[X]$, the support of X, then  \\
\begin{center}
\qBrd[3in]{applegreen!40}{$$E\left(e^{tX} \right)= \sum_{\left\{\HLTW{x}\in \support[X] \right\}} \HLTW{e^{tx}}  \; \HLTY{\pmf_{_X}(x)}.$$\\
\vspace{-.1in}
}
\end{center}
\vspace{.1in}
...
The probability mass function of $\text{Binomial}(n, p)$ is given by
$$\pmf(x):= {n \choose x} p^x (1-p)^{n-x}, \text{ for } x = 0, 1, \ldots,  n \text{ and } {0 < p <1 }.$$\\
$E(X)= n\times p $ and $\text{Var}(X)= np(1-p)$.
}
\end{frame}
