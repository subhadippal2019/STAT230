...
\TransitionFrame[antiquefuchsia]{\Large  Moment Generating Function \HLTW{\text{(mgf)}} }  
...
\qBrd[4.5in]{olive!40}{\sqBullet{olive}
If it exists, the mgf  can be used to obtain the moments of a random variable in the following way: \\
\begin{center}
\qBrd[3.5in]{applegreen!40}{
$$ \frac{d}{dt}\left\{  M_X(t)\right\}\mid_{t=0}= E(X)  $$
}\\
\qBrd[3.5in]{amber!40}{
$$ \frac{d^{k}}{dt^k}\left\{  M_X(t)\right\}\mid{t=0}= E(X^k) \text{ for }k=1 , 2, \ldots  $$
}
\end{center}
}
\end{frame}
