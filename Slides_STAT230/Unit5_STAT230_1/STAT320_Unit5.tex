...
$$\HLTW{\HLTY{\pmf_{_X}(x)}= P(X=x) }\text{ for all } x\in \support[X]$$
}
...
$$\HLTY{\HLTW{ \displaystyle P(X=x)= \pmf_{_X}(x)} \text{ for } \HLTW{x\in \support[X]}.}$$
...
$$\DBX{ \displaystyle \mu_{_X}= E\left(\HLTW{X}\right)= \sum_{\left\{ \HLTEQ[teal!40]{\HLTW{x}\in \support[X]} \right\}} \HLTW{x}  \; \HLTY{\pmf_{_X}(\HLTW{x})}.}$$
}
\end{frame}
...
\begin{center}
\qBrd[4.2in]{ube!40}{ If the random variable $X$ has the probability mass funciton $\HLTY{\pmf_{_X}(x)}$ for all $ \HLTEQ[teal!40]{\HLTW{x}\in \support[X]}$, the support of X, then  \\
\begin{center}
