...
\vspace{.2in}
\qBox{{\bf Notation:} We will use the notation $\support[X]$ or simply $\support$ to denote the support of a random variable X.}
...
If $X$ is a random variable with pmf $\pmf_{_X}(x)$, then the expected value (the mean) of $X$ denoted by $E(X)$ or $\mu_{_X}$ is given by
...
\begin{frame}
\define{\tiny The  Expected Value of a Function of a Discrete Random Variable}{
Let $h(x)$ be any$^{*}$ function,  then the expected value of $ h(X)$ is defined as 
