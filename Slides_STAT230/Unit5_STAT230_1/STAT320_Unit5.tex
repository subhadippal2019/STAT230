...
$$\HLTY{ \displaystyle P(X=x)= \pmf_{_X}(x) \text{ for } x\in \support[X].}$$
The cumulative distribution function (cdf), denoted by $\cdf_{_X}(\cdot)$ is the following quantity
$$\DBX{ \displaystyle \cdf_{_X}(a)= P(X\leq a)= \sum_{\{x\leq a: x\in \support[X]\}}\pmf_{_X}(a)}$$	
}
\end{frame}
...
$$\DBX{ \displaystyle \mu_{_X}= E\left(\HLTW{X}\right)= \sum_{\left\{\HLTW{x}\in \support[X] \right\}} \HLTW{x}  \; \HLTY{\pmf_{_X}(\HLTW{x})}.}$$
}
\end{frame}
...
$$\DBX{ \displaystyle  E\left(\HLTW{h(X)} \right)= \sum_{\left\{\HLTW{x}\in \support[X] \right\}} \HLTW{h(x)}  \; \HLTY{\pmf_{_X}(\HLTW{x})}.}$$
}
\end{frame}
...
$$\DBX{ \displaystyle \text{Var}(X):= E(X^2)- \left(\HLTW{E(X)} \right)^2}$$\\
\vspace{-.1in}
}
...
\begin{center}
\qBrd[3in]{applegreen!40}{$$E\left(e^{tX} \right)= \sum_{\left\{\HLTW{x}\in \support[X] \right\}} \HLTW{e^{tx}}  \; \HLTY{\pmf_{_X}(\HLTW{x})}.$$\\
\vspace{-.1in}
}
\end{center}
\vspace{.1in}
