...
\vspace{-.1in}
\begin{center}
\qBrd[3in]{antiquefuchsia!60}{
$\displaystyle F_{_{X,Y}}(\HLTY{x},\HLTY{y}):= P\left(X\leq \HLTY{x}, Y\leq \HLTY{y} \right) .$}
\vspace{.1in}
\end{center}
}
\vspace{.2in}
...
$$ P\left(X\leq \HLTY{x}, Y\leq \HLTY{y} \right)= \mathop{\sum\sum}_{\left\{  \Col{{ \HLTW{s}\leq \HLTY{x}, \HLTW{t} \leq \HLTY{y} }, {\text{ where } (s,t)\in \HLTW{\support[ X,Y]}}}  \right\}} \pmf_{_{X,Y}}\left(\HLTW{s}, \HLTW{t}\right) $$
\vspace{-.2in}
}
\vspace{.1in}
}
...
$$\pmf_{_X}(\HLTY{x})=  \sum_{\left\{\HLTW{t}: (\HLTY{x}, t)\in \support[XY]\right\}}  \pmf_{_{X,Y}}(\HLTY{x}, \HLTW{t})$$
}
\vspace{.1in}

\qbx[4.2in]{babyblueeyes!40}{
The marginal probability mass function of $X$ is given by
$$\pmf_{_Y}(\HLTY{y})=  \sum_{\left\{\HLTW{s}: (s,\HLTY{y})\in \support[XY]\right\}}  \pmf_{_{X,Y}}(\HLTW{s}, \HLTY{y})$$
}
...
$$ P\left(X\leq \HLTY{x}, Y\leq \HLTY{y} \right)= \mathop{\int\int}_{\left\{ \Col{{\HLTW{s}\leq \HLTY{x}, \HLTW{t} \leq \HLTY{y} },{\text{ where } (s,t)\in \HLTW{\support[ X,Y]}}}\right\}} f_{_{X,Y}}(\HLTW{s}, \HLTW{t}) ds\; dt $$
}
\end{frame}
...
\vspace{.1in}
\qBrd[4.6in]{teal!30}{\tiny
The conditional distribution of X given Y=2 is calculated for each possible value of X using $\pmf_{_{X\mid Y=2}}(x)= \frac{\pmf_{_{X, Y}}(x, 2)}{ \pmf_{_{Y}}(2)}$. The table below shows the results\\
\begin{tabular}{|l|l|}
\hline
