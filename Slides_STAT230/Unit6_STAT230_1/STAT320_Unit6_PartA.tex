...
$$\HLTW{\support[X]=\{x : f_{_X}(x)>0\}}$$
}

\qBrd[4in]{olive!30}{
Support of a random variable refers to the all possible value or outcome of the random variable.  Technically,  if a point is a  possible outcome, the corresponding value of the pdf is positive.  
}
\vspace{1in}
\end{frame}
...
\qBrd[4.2in]{applegreen!30}{$$\HLTW{ \displaystyle F_{_X}(\HLTY{x})= \int_{-\infty}^{\HLTY{x}} f_{_X}(y) dy }$$}\\
\vspace{.1in}
\qBrd[4.2in]{babyblue!30}{$$\HLTW{\displaystyle f_{_X}(x)= \frac{d}{dx} F_{_X}(x)}$$}
\end{frame}
