...
\begin{enumerate}
\item what is the conditional probability that both flips shows up Heads, given that the first  flip is a Head? 
\item what is the conditional probability that both flips results in Heads,  given that  at least one flip shows up Head?
\end{enumerate}
}\\
%\pause
%\vspace{1in}
...
\section{ Partition of an Event (Reminder From Unit1) }
...
\vspace{-.1in}
\qbx[4.5in]{teal!40}{
\Exmpl{teal}{}   If A and B are independent events with \HLTW{P( A) = 0.5} and \HLTW{P( B) = 0.2}, find the following:
\begin{enumerate}
...
\section{A Few Miscellaneous Examples}


\TransitionFrame[antiquefuchsia]{\Large A Few Miscellaneous Examples   }
...
$\HLTEQ[yellow]{ P(B\mid A)=0.9, P(\Not{B}\mid A^c)=0.9 \text{ and  } P(A)=0.01}$
 $$P(A\mid B)= \frac{P(B\mid A)P(A)}{  P(B\mid A)P(A)+ P(B\mid \Not{A})P(\Not{A}) }= \frac{(0.9)(0.01)}{(0.9)(0.01)+ (0.1)(0.99)}=8.33\%$$
}
...
%\begin{enumerate}
%\item What is the probability that that the car repair is satisfactory given that it is late?
%\item What is the probability that that the car repair is satisfactory given that it is on time?
%\end{enumerate}
%}\\
%%\pause
%
...
\vspace{-.1in}
\qbx[4.5in]{amber!40}{
\Exmpl{amber}{}    A card is selected at random from an ordinary deck of 52 playing cards. If E is the event that the selected card is an ace and F is the event that it is a spade, then E and F are statistically independent.
}\\
\pause
