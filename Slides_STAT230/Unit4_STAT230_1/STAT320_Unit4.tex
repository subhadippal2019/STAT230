...
\qBrd[3in]{green!40}{$P(E)= P(E\cap F) + P(E\cap \Not{F}) $  \\$ P(E)= P(E\mid F)P(F) + P(E\mid  \Not{F}))(\Not{F}) $}
\end{center}
}
\end{frame}
...
\begin{center}
%\qBrd[2in]{applegreen!40}{$ E= (E\cap F) \cup (E\cap \Not{F}) $}\\
\qBrd[3in]{green!40}{$$\displaystyle P(E)=\sum_{j=1}^{k}  P(E\mid F_j)P(F_j)  $$}
\end{center}
}
\end{frame}
...
\begin{theorem}
Let $F_1, F_2, \ldots,  F_k$ be a set of mutually exclusive and
...
ipped, and all 4 outcomes are assumed to be equally likely.  If E is the event that the first coin lands on heads and F the event that the second lands on tails, then E and F are independent, since
...
\begin{frame}\frametitle{Generalized Definitioon of Statistical Independence }
